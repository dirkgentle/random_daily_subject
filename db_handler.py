...
class DatabaseHandler:
    ...
    def create_tables_db(self):
        ...
        self._cursor.execute('''CREATE TABLE IF NOT EXISTS bodies (
            id TEXT PRIMARY KEY,
            body TEXT NOT NULL,
            count INTEGER,
        ...
        self._cursor.execute('''CREATE TABLE IF NOT EXISTS submitted (
            id TEXT PRIMARY KEY,
            date TEXT NOT NULL,
            weekday INTEGER,
        ...
        self._cursor.execute('''CREATE TABLE IF NOT EXISTS holidays (
    ...
    def update_title(self, topic, is_holiday=0, is_special=0):
        self._cursor.execute(
            'SELECT is_active FROM titles WHERE id=?', (topic['id'],)
        )
    ...
    def update_holiday(self, topic):
        self._cursor.execute(
            'SELECT * FROM holidays WHERE title_id=?', (topic['id'],)
        )
        db_topic = self._cursor.fetchone()
        if not db_topic:
            self._cursor.execute(
    ...
    def get_latest_submissions(self, n=6):
        self._cursor.execute(
            'SELECT title_id FROM submitted '
            'ORDER BY DATETIME(date) DESC LIMIT ?',
            (str(n),)
        )
    ...
    def get_title(self, title_id):
        self._cursor.execute(
            'SELECT title FROM titles WHERE id=?', (title_id,)
        )
        return self._cursor.fetchone()
