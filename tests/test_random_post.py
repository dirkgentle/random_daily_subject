...
from random_daily_subject.models import Title
...
def test_choose_random_body(db_with_history: DBHandler, sample_title: Title) -> None:
    n_iters = 10
    bodies = sample_title.bodies

    random_choices = []
    for _ in range(n_iters):
        random_choices.append(
            random_post.choose_random_body(db_with_history, sample_title.id)
        )
